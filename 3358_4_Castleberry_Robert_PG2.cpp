...
void setVals(vector<vector<int> > &matrix)
{
    ...
    for (int i = 0; i < matrix.size(); i++)
    {
        for (int j = 0; j < matrix.size(); j++) // Cam Edit: Changed matrix[i].size() to matrix.size()
        {
            ...
            {
            	temp = rand() % 150;
            	if(find(storage.begin(),storage.end(), temp) != storage.end())	//Chuck edit: checks the storage array for the random number
            	{
            		distinct = false;	//if number is found set bool to false									
				}
				else
				{
					distinct = true;	// sets bool to true if random number has not been used
					storage.push_back(temp);	//push the (now) used random number into the array
				}
            } while(distinct == false);

            // Cam Edit: If a distinct random int is generated and verified, it is assigned
            matrix[i][j] = temp;	//adds random number to matrix
